...
function renderCatalogGrid(list) {
  ...
  if (list.length === 0) {
    grid.innerHTML = `<p class="col-span-full text-xs text-slate-500 text-center py-8 font-mono">No spare parts in store catalog.</p>`;
    return;
  }
    ...
    <div class="bg-slate-950 p-4 rounded-xl border border-slate-800/80 hover:border-blue-500/30 transition flex flex-col justify-between space-y-4">
      ...
      <div class="p-2.5 bg-slate-900 rounded-lg border border-slate-800 space-y-1 font-mono text-[10px]">
        <div class="flex justify-between">
          ...
          <span class="font-bold text-white">${window.UTILS.formatCurrency(p.retailerPrice)}</span>
        </div>
        <div class="flex justify-between">
          <span class="text-slate-500">Retailer Reward:</span>
          <span class="font-black text-[#FF6B00]">+${p.retailerPoints} Pts</span>
        </div>
      </div>
...
function calculateRequiredPoints(value) {
  const reqInput = document.getElementById("redeem-required-pts");
  const balInput = document.getElementById("redeem-user-balance");
  
  if (reqInput) reqInput.innerText = value ? `${value} Pts` : "0 Pts";
  if (balInput) balInput.innerText = `${currentRetailerSession.points || 0} Pts`;
}

async function handleRedemptionSubmit(e) {
  e.preventDefault();
  const amt = Number(document.getElementById("redeem-amount").value);
  const method = document.getElementById("redeem-method").value;
  const details = document.getElementById("redeem-details").value.trim();

  if (!amt) {
    window.UTILS.showToast("Please choose a cash-out value option.", "warning");
    return;
  }

  if ((currentRetailerSession.points || 0) < amt) {
    window.UTILS.showToast("Insufficient wallet balance for this payout option.", "error");
    return;
  }

  window.UTILS.showLoader("Transmitting redemption request...");
  try {
    // Deduct points
    await window.API.updateUserPoints(currentRetailerSession.email, -amt);
    ...
    const claim = {
      ...
      firmName: currentRetailerSession.firmName,
      productID: "RED-CASH",
      productName: `Cash Redemptions Payout: ${method.toUpperCase()}`,
      quantity: 1,
      pointsCalculated: -amt
    };

    await window.API.submitPurchaseClaim(claim);
    window.UTILS.showToast("Cash-out request successfully submitted! Paytm transfer processing.", "success");
...
async function loadRedemptions() {
  ...
  try {
    ...
    const myRedemptions = claims.filter(c => 
      c.email.toLowerCase() === currentRetailerSession.email.toLowerCase() && 
    ...
    if (myRedemptions.length === 0) {
      tbody.innerHTML = `
        <tr>
          <td colspan="3" class="py-4 text-center text-slate-500 font-mono">No previous cash redemptions requested.</td>
        </tr>
      `;
      return;
    }
...
window.calculateRequiredPoints = calculateRequiredPoints;
